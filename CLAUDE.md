...
* **Trailing SL survives a restart (2026-07-11).** `zerodha.trailing_sl_state` (migration `0012`) is the live, mutable current-SL snapshot for active trailing orders — unlike `zerodha.orders`, this row IS updated in place on every ratchet (`ExecutionRepository.upsert_trailing_sl_state`). `src/execution/trailing_sl.py::rehydrate_trailing_sl()` runs once at `zerodha-monitor` startup (`src/monitor/service.py::main`, before the poll loop starts) and resumes every row still marked `active`, so a process restart mid-trail no longer silently strands the stop. `cancel_trailing_sl` marks the row inactive so a later restart doesn't resurrect a cancelled trail. Both the DB write and the in-memory task management degrade gracefully with no running event loop or no DB configured — never raises into the caller.
