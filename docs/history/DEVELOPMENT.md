...
- **`INDmoneyBroker.search_instruments(query, source, limit)`** (`src/brokers/indmoney.py`) — substring search over the cached instrument CSV; prefix matches (`RELIANCE` for query `REL`) rank before mid-string matches. Originally omitted `security_id` from results (revised below — 2026-07-11) since callers were meant to re-resolve by symbol text; that assumption turned out to be wrong for options and was corrected.
...
### Follow-up — fixed weekly-options symbols collapsing to month-only, parallelized search (2026-07-11)

**Bug found from a screenshot**: searching a NIFTY strike (e.g. "27500") only ever surfaced one result per month (`NIFTY-JUL2026-27500-PE`, `NIFTY-AUG2026-27500-PE`, ...) with no specific weekly expiry dates — even though NIFTY is one of only two indices (with SENSEX) that still runs a weekly options series post the Nov-2024 SEBI rationalization (`src/market/calendar.py::_MONTHLY_ONLY_INDICES` excludes both).

**Root cause**: INDstocks' `TRADING_SYMBOL` column is month-granular for index options — every Thursday's weekly contract in a given month can share the exact same `TRADING_SYMBOL` string, differing only by the separately-documented `EXPIRY_DATE`/`SECURITY_ID` columns (confirmed against api-docs.indstocks.com/instruments/'s verbatim schema). `search_instruments()`'s original `seen.add(sym)` dedup keyed purely on that display string, so it silently kept only the first weekly row per month and dropped the rest. `resolve_security_id()` had the same ambiguity — matching on symbol text alone, it would return whichever row the CSV listed first for a shared `TRADING_SYMBOL`, a real correctness risk (a `/buy` could silently execute against the wrong week's contract).

**Fix:**
- `search_instruments()` now dedupes by `security_id` (the one column guaranteed unique per contract), not the display symbol, and returns `security_id` + `expiry` (from `EXPIRY_DATE`) per result. Results with the same symbol name now sort chronologically by expiry.
- `execution.service.search_symbols()`'s cross-source dedup updated to match (key on `security_id`, fallback to symbol text only when no id is present).
- **Web (`trade.html` + `server.py`)**: the autocomplete dropdown now shows each result's expiry date so same-named weekly contracts are visually distinguishable; picking a result carries its `security_id` through `payload()` (invalidated if the user edits the text afterward — checked by exact string match against `selected.symbol`). `/trade/place` trusts a client-supplied `security_id` and skips the ambiguous symbol-text `resolve_symbol()` call entirely when one is present. If a user types an option-shaped symbol without picking from the dropdown, the confirm screen shows an explicit warning that the wrong expiry could be picked.
- **Telegram `/search`**: still symbol-text-only (no picker in a chat), so it now shows each result's expiry inline and, when a symbol matches more than one row, prints an explicit warning naming the ambiguous symbols and pointing to the web `/trade` page instead of letting `/buy`/`/sell` on that name look like it's unambiguous.
- **Perf**: `search_symbols()` was awaiting the equity and fno instrument-master lookups sequentially — switched to `asyncio.gather` so both fetch concurrently, roughly halving cold-cache (post-TTL-expiry or post-restart) worst-case latency. Added `INDmoneyBroker.warm_instrument_cache()` and wired it into both long-running processes' startup (a background thread + `asyncio.run` in `server.py`, mirroring the existing `_refresh_calendar_background()` pattern; a fire-and-forget task in `telegram_admin/main.py`'s `post_init`) so the instrument CSVs are already warm before a user's first search of the day.

12 new tests, including two that assert concurrency directly (measuring elapsed time against a mocked slow fetch) and one that reproduces the exact collapsing bug with three same-named, differently-expiring rows. Full suite: 2101 passed, 6 skipped.
