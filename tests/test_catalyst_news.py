...
class TestGetSymbolNews:
    def _call(self, symbol="INFY", count=10, monkeypatch=None, ticker=None):
        import src.catalyst.news as news_mod
        with news_mod._LOCK:
            news_mod._CACHE.clear()
        if monkeypatch and ticker is not None:
            monkeypatch.setattr(news_mod, "_get_ticker", lambda t: ticker)
        from src.catalyst.news import get_symbol_news
        return get_symbol_news(symbol, count)

    def test_returns_expected_keys(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([_headline("Infosys beats Q1")])
        result = self._call(monkeypatch=monkeypatch, ticker=ticker)
        for key in ("symbol", "yf_ticker", "count", "headlines"):
    ...
    def test_yf_ticker_resolved(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([])
        result = self._call(symbol="INFY", monkeypatch=monkeypatch, ticker=ticker)
        assert result["yf_ticker"] == "INFY.NS"

    def test_count_matches_headlines_length(self, monkeypatch):
        import src.catalyst.news as news_mod
        items = [_headline(f"Article {i}") for i in range(5)]
        ticker = _mock_ticker(items)
        result = self._call(count=5, monkeypatch=monkeypatch, ticker=ticker)
        assert result["count"] == len(result["headlines"])

    def test_count_limit_respected(self, monkeypatch):
        import src.catalyst.news as news_mod
        items = [_headline(f"Article {i}") for i in range(20)]
        ticker = _mock_ticker(items)
        result = self._call(count=3, monkeypatch=monkeypatch, ticker=ticker)
        assert result["count"] <= 3
        assert len(result["headlines"]) <= 3

    def test_headline_has_sentiment(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([_headline("Infosys beats Q1 estimates")])
        result = self._call(monkeypatch=monkeypatch, ticker=ticker)
        h = result["headlines"][0]
        assert h["sentiment"] in ("POSITIVE", "NEGATIVE", "NEUTRAL")

    def test_empty_news_list(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([])
        result = self._call(monkeypatch=monkeypatch, ticker=ticker)
    ...
    def test_nse_prefix_resolved(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([])
        result = self._call(symbol="NSE:TCS", monkeypatch=monkeypatch, ticker=ticker)
        assert result["yf_ticker"] == "TCS.NS"

    def test_index_symbol_resolved(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([_headline("Market rises")])
        result = self._call(symbol="NIFTY", monkeypatch=monkeypatch, ticker=ticker)
        assert result["yf_ticker"] == "^NSEI"

    def test_cache_returns_same_object(self, monkeypatch):
        import src.catalyst.news as news_mod
        ticker = _mock_ticker([_headline("Test")])
        monkeypatch.setattr(news_mod, "_get_ticker", lambda t: ticker)
        with news_mod._LOCK:
            news_mod._CACHE.clear()
        from src.catalyst.news import get_symbol_news
        r1 = get_symbol_news("INFY", 10)
        r2 = get_symbol_news("INFY", 10)
        assert r1 is r2
